...
subtest 'testing array - no diff' => sub {
	...
	ok !diff([1, undef], [undef, 1]), 'array with two elements is equal to array with two elements (different order)';
...
subtest 'testing array - diff' => sub {
	...
	ok diff(['abc', 1], ['def', 1, undef], \$out), 'arrays differ when element from first array has is not found in the second array';
